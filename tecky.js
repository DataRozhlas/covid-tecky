...
const pltLines = [{
  color: '#de2d26',
  value: Date.parse('2021-01-10'),
  width: 1,
  label: {
    text: 'Není to ničí vina',
    style: {
      color: '#de2d26',
      fontWeight: 'bold'
    },
...
fetch('https://onemocneni-aktualne.mzcr.cz/api/v2/covid-19/umrti.csv')
  ...
  .then((resp) => {
    ...
    Highcharts.chart('mrtvi-tecky', {
      ...
      credits: {
        text: `data (k ${parseInt(ld.split('-')[2])}. ${parseInt(ld.split('-')[1])}.): ÚZIS`,
      ...
      yAxis: {
        ...
        labels: {
          formatter() {
            ...
            return `${tStamp.getDate()}. ${tStamp.getMonth() + 1}. ${tStamp.getFullYear() - 2000}`;
          },
        },
        gridLineWidth: 0,
      ...
      series: [{
        ...
        tooltip: {
          ...
          pointFormatter() {
            ...
            const dte = `${parseInt(sel[2].split('-')[2])}. ${parseInt(sel[2].split('-')[1])}. ${sel[2].split('-')[0]}`;
            return `${dte}<br>${sex[sel[3]]} ${sel[4]} let, okres ${muni[sel[5]]}`;
          },
        },
      }],
